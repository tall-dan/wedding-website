task create_clair_sheet: :environment do
  ...
  Invite.reception_invites.where(status: 'accepted').order(:table_number).includes(:guest).group_by(&:table_number).map do |table_number, invites|
...
task create_alpha_order_guest_list: :environment do
  ...
  csv = CSV.new(outfile, write_headers: true, headers: headers)
  guests = Guest.includes(:invites).where(invites: { event_id: Event.reception, status: 'accepted' }).order(:last_name, :first_name)
