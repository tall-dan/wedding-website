...
after 'deploy:finished', :restart_eye do
  on roles(:app) do
    ...
    execute "cd #{release_path} && RAILS_ENV=production bundle exec eye stop wedding_website"
    execute "cd #{release_path} && RAILS_ENV=production bundle exec eye start wedding_website"
  end
end
